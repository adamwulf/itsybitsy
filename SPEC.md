...
- `ib team create @<name>` fails if `<name>` is a **reserved word**. The reserved set is the **union of three distinct sources**, NOT a single constant — `BARE_RENDERED_SENTINELS` alone is insufficient (it currently contains only `@watchdog`, `ib-commands.ts`). The implementer must reserve, with the leading `@` stripped: (1) the system coordinator id — `SYSTEM_AGENT_ID` (`= "system"`, `hooks/shared.ts`); (2) the literal `coordinator` (already hard-refused as a repo name in `registry.ts`, so teams must mirror it); (3) every member of `BARE_RENDERED_SENTINELS` (`ib-commands.ts` — currently `watchdog`). Derive each from its source constant at build time so the spec and code cannot drift; do not hard-code a literal list. (An earlier draft listed `telegram` here — **dropped**: no `@telegram` *send-target sentinel* exists in source; `channels.telegram` is a config key, not an address, so it is not a collision risk.)
...
**Eager prune — the CLI teardown path (`archiveAgent`).** The single correct hook for the membership *write* is `archiveAgent` in `agent-lifecycle.ts` — the function `ib merge`, `ib kill`, `ib nuke`, and `nukeAllAgents` all route through (verified: kill→teardownAgent→archiveAgent; nuke/nukeAll→nukeAgentList→archiveAgent per agent; merge→archiveAgent post-merge), and the same place that calls `deleteAgentOutbox()` / `deleteAgentTransient()`.

But **notice emission must be separated from the membership write**, because `nukeAllAgents` invokes `archiveAgent` **once per agent** — so a per-agent notice emitted *inside* `archiveAgent` reproduces exactly the O(N²) storm §16.4.2 forbids. The split:

- **`archiveAgent` always performs the membership prune** (remove this agent's id from every team it's in, under the lock, §16.2) — this part is per-agent and unconditional.
- **Notice emission is governed by a `suppressLeaveNotice` (batch-mode) parameter** on `archiveAgent`. Single-agent callers (`ib kill`/`ib merge`/`ib nuke` of one leaf agent) leave it false → `archiveAgent` emits the normal per-agent leave notice after its own prune. Bulk callers (`nukeAllAgents`, and any multi-agent teardown — see the `ib nuke <manager>` row) pass it **true**, so the per-agent loop only prunes membership and emits **nothing**; the bulk caller then emits **one coalesced notice per affected team** (§16.4.2) computed from the set of removed ids, after the loop completes.

| Departure path | Membership prune | Notice |
|---|---|---|
| `ib merge` (single) | in `archiveAgent` | one per-agent leave notice |
| `ib kill` (single leaf) | in `archiveAgent` | one per-agent leave notice |
| `ib nuke` (single leaf) | in `archiveAgent` | one per-agent leave notice |
| `ib nuke <manager>` (manager **+ its descendants** — structurally bulk) | in `archiveAgent` per agent, `suppressLeaveNotice: true` | **coalesced** per-team notice from the caller (§16.4.2) |
| `nukeAllAgents` (bulk) | in `archiveAgent` per agent, `suppressLeaveNotice: true` | **coalesced** per-team notice from the caller (§16.4.2) |
...
| `src/agent-lifecycle.ts` | `archiveAgent` always prunes the torn-down agent from all teams (under `.teams.lock`), alongside `deleteAgentOutbox`. New `suppressLeaveNotice` (batch-mode) parameter: false (default, single-agent) → emit the per-agent leave notice; true (bulk callers) → prune only, caller emits the coalesced per-team notice afterward (§16.5/§16.4.2). `ib pause` deliberately does NOT prune. |
