...
A type can build on top of another by declaring `inherits: <parent-type>` in its frontmatter. The parent name is a filename (no `.md`) in the same `~/.itsybitsy/agent-types/` directory. The chain is resolved parent-first; scalar fields declared in the child **override** the parent, while `permissions.allow` and `permissions.deny` lists are **unioned** across the entire chain (deduplicated via `Set`). The `paths:` (`allowRead` / `allowWrite` / `deny`) and `sandbox:` list fields are likewise **unioned** across the chain; `repos` **replaces** (not merge) when the child declares it. The markdown bodies **concatenate** root-first: oldest ancestor body first, each descendant's body appended below, joined by blank lines. Empty bodies in the chain are skipped, so a middle ancestor that only customizes frontmatter doesn't introduce stray whitespace.

**Multi-level chains** are fully supported. Given `researcher inherits assistant` and `assistant inherits manager`, `loadAgentType("researcher")` walks all three files and produces a merged type with: permissions unioned across manager + assistant + researcher, scalar fields taking the nearest-present value (descendants winning), and a body of `manager body \n\n assistant body \n\n researcher body`. Cycles at any depth (`A → B → C → A`) and missing parents are detected at `ib watch` startup.

**`{{#if}}` block boundaries.** Each file should keep its `{{#if condition}}...{{/if}}` blocks self-contained. Under body concatenation the interpolator sees the joined string, so an `{{#if}}` opened in an ancestor and closed in a descendant will technically resolve — but it silently couples the two files together and is confusing to read. Treat each type file's body as a complete, independent template.
...
When enabled, repository agents and per-repository coordinators run inside the itsybitsy `sandbox-exec` profile and egress proxy; the resolved path configuration supplies the coarse kernel rules, while hooks enforce agent-type tool permissions and fine-grained paths. Failure to establish the enabled wrapper or proxy stops launch. With explicit `false`, only the itsybitsy wrapper, proxy, and kernel-denial collector are skipped, restoring pre-sandbox process behavior. Hooks remain active and resolve every tool allow/deny without native user prompts in both modes.

CLI details are fixed by that contract: Codex/Fugu always use `-a never -s danger-full-access`; their native sandbox is off in both modes and hooks handle permissions. Existing sessions need a resume or sandbox refresh to pick up changed launch flags. Agy always uses `--dangerously-skip-permissions --mode=accept-edits`, retaining its fail-closed hooks in both modes. Claude adds `--dangerously-skip-permissions` only when the kernel is enabled; when disabled it emits neither that flag nor `--permission-mode`. Claude PreToolUse explicitly allows or denies before native permission resolution, checking `permissions.deny` first, so neither mode prompts. The global `@system` coordinator remains outside this rollout.
