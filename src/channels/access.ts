...
/** Path to the chat-id state file. Lives alongside access.json so both paths
 *  share the same setStateDir override for tests. */
function chatIdPath(): string {
  return join(defaultStateDir(), "chat-id");
}

/**
 * Persist the resolved Telegram chat ID. `ib watch` writes this after step 2
 * of the Telegram subsystem boot succeeds; `ib tgsend` reads it instead of the
 * (now-removed) `channels.telegram.chat_id` config key.
 *
 * The file is plain text with a trailing newline. Phase A throwaway plumbing —
 * Phase B will replace `tgsend`'s in-process send with an outbox queue, at
 * which point this helper goes away.
 */
export async function writeChatId(id: string | number): Promise<void> {
  ensureStateDir();
  const idStr = String(id).trim();
  await Bun.write(chatIdPath(), idStr + "\n");
}

/**
 * Read the persisted chat ID written by `ib watch`. Returns null when the
 * file is absent or empty after trimming. Callers (e.g. `ib tgsend`) surface
 * a "Telegram chat not yet resolved" error when null is returned.
 */
export async function readChatId(): Promise<string | null> {
  const path = chatIdPath();
  const file = Bun.file(path);
