...
2. **Codex tmux overrides:** a small `parse-state-codex.ts` (or codex branch) for the override states the hooks can't capture (rate-limited, api_error, compacting) by matching codex's actual UI strings (gathered empirically in Phase 5 — formerly Phase 4). **Partially implemented (2026-05-31):** the `parseState()` dispatcher in `src/parse-state.ts` now detects codex output via `isCodexTmuxOutput()` (banner `"OpenAI Codex (v"` OR a tail `"<text> · <path>"` status bar) and routes to `parseCodexState()`, which recognizes the idle-prompt shape (a bare `›` line near the tail + status bar at the very end), the `WAITING` marker, the `"I HAVE COMPLETED THE GOAL"` completion sentinel, and Codex's active `Working (... esc to interrupt)` marker. **Still deferred:** override-state detection for `rate_limited`, `api_error`, and `compacting` (codex UI strings for these states haven't been captured yet; the claude-shaped `isCompacting()` / `isRateLimited()` / `isApiError()` helpers in `src/agents.ts` carry TODO markers pointing back here). Snapshot evidence: `/Users/adamwulf/Developer/muse/muse-ios/.ittybitty/agents/agent-2a4815af/debug-logs/snapshot-2026-05-31T19-31-32-456Z-waiting.txt` previously parsed as `state: 'unknown'` and now parses as `state: 'waiting'` (covered by `tests/parse-state.test.ts`).
