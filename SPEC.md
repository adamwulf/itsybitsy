...
- **codex path:** generates a codex-shaped `start.sh` via `buildCodexStartContent()` in `src/codex-spawn.ts`. The codex branch:
  ...
  3. Puts **no role text on the launch line**. The codex SessionStart hook (`ib hooks codex-session-start`, `src/hooks/codex-session-start.ts`) returns the role instructions as `hookSpecificOutput.additionalContext`, the same way the Claude session-start hook does. The text is `buildAgentRoleBody()` for the agent's frozen `meta.json` and worktree: the session-start template (the `<ittybitty>` wrapper stripped via `stripIttybittyWrapper()`) plus the skills catalogue. Codex (verified against the 0.154.0 source) runs SessionStart on startup, resume, `/clear` and after every compaction (`source` = `startup` / `resume` / `clear` / `compact`; our matcher is `.*`) and records the text as a developer message before that turn's user input, so the agent has it before its first turn and again after each compaction (compaction drops the old copy). Codex cuts `additionalContext` above 2,500 estimated tokens (bytes ÷ 4) down to a preview plus a file path; the SessionStart registration sets `additionalContextLimit=0` so the whole role text reaches the model. Hook stdout has no size cap. **Nothing is written into the worktree**: an earlier design wrote a generated `<worktree>/AGENTS.md`, which clobbered a repo-tracked `AGENTS.md` (codex reads one `AGENTS.md` per directory).
...
| codex / fugu | `AGENTS.md` (`AGENTS.override.md` wins in the same directory); not `CLAUDE.md` by default (only if listed in `project_doc_fallback_filenames`) | `~/.codex/AGENTS.md` | SessionStart hook injection |
...
- **codex role instructions on resume:** the SessionStart hook runs with `source: "resume"` and adds the role text built from the current frozen meta (so `ib sandbox refresh` policy changes are included) before the first resumed turn. Copies from earlier in the session stay in the resumed history until the next compaction drops them.
