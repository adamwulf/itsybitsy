...
- The shell scanner provides early denials for recognizable paths; it is not a complete shell parser or kernel boundary. Enabled Seatbelt profiles supply the outer OS boundary. With explicit false, pre-sandbox process behavior is restored while the same hooks remain authoritative for tool and path decisions.
- Enabled launches and resumes require a valid profile and proxy, failing closed on setup or platform errors. Disabled launches omit the itsybitsy wrapper, proxy, and kernel-denial collector, but do not restore native user approval prompts: Codex/Fugu keep `-a never -s danger-full-access` with no native sandbox; agy keeps `--dangerously-skip-permissions --mode=accept-edits`; Claude emits neither `--dangerously-skip-permissions` nor `--permission-mode`, and its PreToolUse hook explicitly allows or denies before native permission resolution with `permissions.deny` checked first.
