...
- **The visible tree renders its selection highlight only when it owns the active selection.** When `sidebarMode === activeSelectionSource`, the visible tree highlights its selection normally. When `sidebarMode !== activeSelectionSource`, the visible tree renders without a highlighted row (the active selection lives in the *other* tree); a `suppressSelection` flag on each tree component gates the reverse-video styling for this case.
- **Visual mirror on toggle (`mirrorSelectionToVisibleTree`).** When `0`/`1` flips `sidebarMode`, the dashboard tries to mirror the active selection across trees for visual continuity. If the active selection is an *agent* that appears as a team-member row in the now-visible Teams tree, that member row is selected (visual only); if the active agent is in multiple teams, the first team (by stable `listTeams()` order) wins. If the active selection is a *team* or a *repo header* / *system coordinator*, the other tree has no counterpart and is rendered in no-selection. The mirror NEVER changes `activeSelectionSource` — it is a render-time hint, not a source-of-truth change.
...
4. Press `1`. `sidebarMode=agents`, `activeSelectionSource` still `=teams` — the team is still the active selection. The Agents tree appears in the sidebar but renders with **no row highlighted** (because the active source is teams). Info / main still drive the team channel.
...
**Implementation references.** `activeSelectionSource` is a public field on `DashboardComponent` (`src/tui/dashboard.ts`). The resolver in `syncSelectedAgent` reads it (line ~1317 — `const teamsActive = this.activeSelectionSource === "teams"`). The mirror lives in `mirrorSelectionToVisibleTree()` and is called from the `0`/`1` handlers. The render path sets `agentTree.suppressSelection` / `teamsTree.suppressSelection` based on the active source (~line 2705 of the render). The ActionCtx setters `setActiveSelectionSource` and `setSidebarMode` are arrow properties on the dashboard so the `@`-fuzzy jump and `g`-go-to-question-agent handlers can write both axes through the ctx.
...
- Each line is **sender-prefixed** like the §16.4 delivery prefix but **without the tmux mechanics**: render `[sent by <fromAgent> in @<team>]: <message>` for an agent sender. For a **human/CLI** sender (`fromAgent === ""`), match §16.4's actual human-form prefix grammar (§16.4): `[sent by user <name> in @<team>]: <message>` **when a `user.name` is configured**, and `[sent by user in @<team>]: <message>` **when it is not** — reusing the §16.4 prefix grammar verbatim so the chat box reads identically to what each member actually saw in their pane (the bare "user" form is only the no-`user.name` case, not the only case). A timestamp gutter (from `ts`) may be shown on the left. [^needs review] Whether to show the `ts` gutter and in what format (relative age vs clock time) is a cosmetic open decision; recommend a short clock time (`HH:MM`) gutter for scannability.
