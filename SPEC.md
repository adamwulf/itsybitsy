...
**Eager prune — the membership *write* and the notice *fan-out* live at two different layers, and the fan-out belongs in the `ib-commands.ts` *command* functions, not in `agent-lifecycle.ts`.** This is the subtle part. `archiveAgent` *and* `teardownAgent` (`agent-lifecycle.ts`) are **repo-local**: `archiveAgent` is pure filesystem archival, and `teardownAgent(repoPath, …)` takes a single `repoPath` and has no `listRepos()` / `RepoEntry` access. The notice fan-out needs the **cross-repo `repos` list** (a team's surviving members may live in other repos), which is only reachable from the **`ib-commands.ts` command layer** (`killAgent`, `mergeAgent`, `nukeAgentList`) — these already call `listRepos()`. So:

- **Membership write → in `archiveAgent` (per agent, unconditional, no notice).** `archiveAgent` removes this agent's id from every team it belongs to, under the `.teams.lock` (§16.2), beside the existing `deleteAgentOutbox()` call. It emits **no** notice and needs no `repos` data — pruning by agent id only reads/writes `teams.json`. It **returns the set of `(team, removed-agent-id)` pairs** it pruned, and `teardownAgent` **threads that return value up** to its caller (its signature changes from `boolean` to carry the pruned pairs).
