...
agy gets user-wide instructions only from its own global `~/.gemini/GEMINI.md` (symlink it to `~/.claude/CLAUDE.md` to share one file; the `~/.gemini` runtime root and the `_all.md` `~/.claude` read floor make both the link and the target readable under the kernel sandbox). Retire agy agents spawned before this change (§18.7).
