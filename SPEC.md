...
agy gets user-wide instructions only from its own global `~/.gemini/GEMINI.md` (symlink it to `~/.claude/CLAUDE.md` to share one file; the `~/.gemini` runtime root and the `_all.md` `~/.claude` read floor make both the link and the target readable under the kernel sandbox). A spawn in a repo with `CLAUDE.md` but no `AGENTS.md`, `GEMINI.md`, `.gemini/GEMINI.md`, or repo-owned `.agents/rules/*.md` prints the same kind of warning as codex (§18.6).
