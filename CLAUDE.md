...
- `newAgent()` calls `ensureAgentTypesDir()` then validates `--type` exists on disk (via `agentTypeExists()`), stores `agentType` and `agentIcon` in meta.json. Permission lookup: coordinators get `permissions.coordinator.*`, all others get `permissions.repo.*`. Type-defined permissions merged from type definition file.
