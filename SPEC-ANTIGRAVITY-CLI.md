...
**Current instructions contract (2026-09-23):** the always-on rule file holds
only the role text and the skills catalogue. itsybitsy copies in no instruction
file: agy reads the repo's `AGENTS.md` / `GEMINI.md` natively and gets
...
| D6 | Instructions go in `<worktree>/.agents/rules/ittybitty-agent.md` with frontmatter `trigger: always_on`. Body = the session-start template (wrapper stripped) + the skills catalogue. (Revised 2026-09-23: the project and user `CLAUDE.md` are no longer inlined; agy reads `AGENTS.md` and `~/.gemini/GEMINI.md` itself.) Never overwrite `AGENTS.md`. No `--agent`. | A bare rule file is ignored; with `trigger: always_on` it loads alongside the repo's own `AGENTS.md`. Workspace custom agents are not discovered; a global one drops the workspace rules. `agy` has no `@file` import. |
...
- `buildAgyRulesFile(ctx)` → `---\ntrigger: always_on\ndescription: itsybitsy agent instructions\n---\n` + body. The body reuses `generateInstructions` + `stripIttybittyWrapper` from the codex path, then appends `buildSkillsSection()`. Factor the shared pieces out of `codex-spawn.ts` rather than copying them. (Revised 2026-09-23: the project and user-global `CLAUDE.md` used to be inlined here; they are not anymore.)
