# Mandatory sandbox rollout

Every agent launch must apply `sandbox-exec`. Agent-type Markdown defines filesystem paths, raw Seatbelt rules, and allowed network domains. It does not define an on/off switch. This replaces the former opt-in rollout and its `sandbox.enabled` instructions.

## Implementation and validation status

The mandatory policy is integrated on `agent/sandbox-exec-feature`. Agent lifecycle and system-coordinator wiring are being integrated and reviewed. This document describes the rollout target; it does not claim the binary has been installed or existing agents migrated.

A Claude worker ran the live kernel filesystem and tmux-socket probes successfully. The offline Claude boot test also passed. That boot test deliberately denies network access and ends by timeout: it does not establish a model reply through the proxy or working MCP servers. Those checks remain required on the final candidate.
...
- `sandbox.enabled` is retired, regardless of its value. Remove the key from every agent-type file that contains it. An omitted sandbox block never authorizes an unsandboxed launch.
...
- If the platform, profile, or proxy cannot support the sandbox, launch fails. There is no unsandboxed fallback.
...
Then test a worker launch with the candidate and matching helper binary. Verify all of the following before broad rollout:
...
- Each deployed CLI and the system coordinator can start under its own profile. The earlier Claude-only boot probe does not prove Codex, fugu, agy, or coordinator integration.

This is a candidate-binary pilot, not a per-type enabled flag: all agents started by the candidate follow the mandatory policy.
...
1. Install the reviewed candidate `ib` onto PATH and restart `ib watch` after the configuration preflight and live validation pass. Ensure lifecycle and proxy helpers resolve to the same candidate version.
...
4. Restart the system coordinator through its dedicated lifecycle path. A pre-feature session cannot be silently treated as sandboxed. Confirm the final implementation verifies the running agent process before adopting an existing session.
5. Verify all remaining live agents were launched by the candidate under a kernel profile. A new value in Markdown or metadata is not proof that an old process changed.

## Failure recovery and limits

Keep the last working binary and the configuration backup until rollout is verified. The new binary has no disable switch. For a failed launch, correct its missing runtime path, domain, or profile rule and retry the supported refresh or restart path. Reverting the entire installation to an older binary is an operator rollback that also restores that version of enforcement; it must not be described as keeping mandatory sandboxing.
...
Spawning agents retain access to the tmux server for lifecycle operations. This is an existing escape from process confinement: they can ask that server to run commands outside their inherited profile. Mandatory wrapping does not close that boundary. Non-spawners have the corresponding socket denied. The system-coordinator configuration files and sealed records require explicit kernel write protection despite its writable home runtime root.
