...
- `meta.spawned_by` (type `SpawnedBy`) records the spawner — `agent_id` is either a real agent ID, or one of two `@`-prefixed sentinels: `@system` (the system coordinator, with `repo_path: null`) or `@<repo-name>` (a per-repo coordinator, with `repo_path` set). The watchdog routes notifications via these sentinels — `@system` writes to the inbox, `@<repo-name>` resolves to that repo's coordinator at notify time so the sentinel survives coordinator restarts. The `notifyManager` / `notifySpawner` precedence in watchdog handlers is mutually exclusive: manager wins if set, else spawner, else nothing.
