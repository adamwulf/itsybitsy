...
if ! ib sandbox-proxy-launch --port "$PROXY_PORT" --domains '<AGENTSDIR>/claude-snapshot/sandbox-domains.txt' --log '<AGENTSDIR>/claude-snapshot/sandbox-proxy.log' --agent-log '<AGENTSDIR>/claude-snapshot/agent.log' --pid-file "$PROXY_PID_FILE" --ready-file "$PROXY_READY_FILE"; then
