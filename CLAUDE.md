...
- **Tmux width derivation:** Only `splitPaneLeftWidth` is stored; the coordinator main width is derived as `terminalWidth - sidebarWidth - 1`. All agent spawn/resume call sites size tmux via `getTmuxWidthForAgent(isCoordinator)` (in `src/tui/layout.ts`) — never compute the `-x` value inline. Coordinators (system + per-repo) span middle+right and use the derived main width; regular agents fit the middle pane only.
