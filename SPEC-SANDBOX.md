...
An explicit false skips Seatbelt, the egress proxy, and kernel-denial collection; hooks continue enforcing `paths`. Enabled launches fail closed if profile or proxy setup fails. Spawn freezes policy in metadata, resume preserves it, and `ib sandbox refresh` applies current type settings in either direction. Global `@system` sandboxing remains deferred.
