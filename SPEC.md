...
3. **Global selection.** What agent / team / repo is currently selected. Drives the info panel, the main area's middle (tmux / chat) pane, and the right pane. Changed by `j`/`k`/`J`/`K` (navigation) and the `@`-fuzzy jump.

Phase 1 (implemented) decouples axis 1 from axis 2: `0`/`1` toggle `sidebarMode` only, `SidebarComponent` keys its rendering off `sidebarMode` (not focus), and the j/k/J/K navigation routes by `sidebarMode` (whichever tree is visible). The legacy `"teams-tree"` focus target is retained in the `FocusTarget` union so existing callers compile, but the dashboard never settles focus there — if a `0`/`1` keypress is observed while focus is already on `"teams-tree"`, focus is reset to `"agent-tree"`. Phase 2 lifts effective selection to a single global resolver decoupled from sidebar visibility. Phase 3 reintroduces `"teams-tree"` as a Tab-reachable target whose inclusion in cycling depends on `sidebarMode`.
