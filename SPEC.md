...
6. **Protected-write block** (mutation tools only — reads are allowed): a write to `<worktree>/.claude/settings*.json` (permission self-escalation), the agy boundary files (`.agents/hooks.json` and the rule file, whose rewrite disables the hook gate on the next resume), the agent's own `<agent-dir>/meta.json` (the hook reads its path lists from it — a self-widening path in hook-only mode), or — for the `@system` coordinator, whose worktree root is its whole `~/.itsybitsy` home — its `agent-types/` dir, `config.json`, `repos.json`, and `layout.json`. All of those are self-widening; agents may read them but must use `ib` to change configuration. The `@system` coordinator's `sealed/` dir is stricter still — **read AND write denied** (it holds the tamper-proof adoption/proxy launch record, so the sandboxed coordinator must not read, forge, or overwrite it); the `@system` config floor adds `<home>/sealed` to the resolved `paths.deny` list, so both the hook resolver and the kernel profile enforce it (deny wins; §12.1.8), and only the unsandboxed `ib watch` parent touches `sealed/`. Protected paths are canonicalized (symlink-resolving) so a symlinked parent cannot defeat the guard.
...
Under the mandatory-sandbox contract the system coordinator's `claude` session is wrapped in `sandbox-exec` at **every** launch path — fresh spawn, session resume, the `R`-key restart (resume), the `/restart` fresh restart, and the resume-failure fresh retry. Sandbox preparation **fails closed before launch** — on non-macOS, a missing `sandbox-exec`, a malformed or absent `_all.md`/`system.md` layer, a profile that will not compile, or an egress-proxy port that cannot bind — so no unsandboxed coordinator ever starts.

The coordinator's worktree root is its whole `~/.itsybitsy` home, which is a writable runtime root. Because that would otherwise let it rewrite its own configuration, the profile adds an **unconditional kernel write-deny** (read preserved) on `agent-types/`, `config.json`, `repos.json`, and `layout.json`, and a **full deny** (read *and* write) on `sealed/`. This is the kernel-enforced companion to the hook's protected-write block (§6.1 step 6): the same deny set is applied by both the generated Seatbelt profile (`generateProfile`) and the shared hook resolver (`resolvePreparedAccess`), via the `@system` config floor adding `<home>/sealed` to `paths.deny`, so the two cannot drift.

**Adopting an already-running coordinator is not proven by metadata.** At a successful sandboxed launch the (unsandboxed) parent records the sandboxed child `claude` process's pid and its OS process-start epoch — plus the egress proxy's pid and epoch — into the coordinator's `sealed/` store, which the sandboxed child cannot read or write. When `ib watch` finds an existing `ib-coordinator` session, adoption re-verifies that the recorded child pid is still alive **and** carries the same process-start generation (a strict epoch-identity check). A match adopts the session as-is; a missing or mismatched record (a pre-feature or otherwise unsandboxed launch, a crashed child whose pane shell persists, a failed profile apply, or a replacement child) **fails closed** with an actionable `/restart` message and never auto-kills. A live pane shell alone is explicitly **not** treated as sandboxed — `sandbox-exec` execs into its target, so a surviving bash pane proves nothing about confinement. A new value in Markdown or metadata is likewise not proof that an already-running process is confined.

*(The entire system-coordinator sandbox wiring described in this section — the all-path `sandbox-exec` wrapping, the kernel config write-denies, and the pid/epoch adoption check — is the intended contract being integrated on `agent/sandbox-exec-feature`; treat it as the target design, not a shipped or gated guarantee — see `docs/SANDBOX-ROLLOUT.md`.)*
