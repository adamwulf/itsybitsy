...
**Observation safety invariant**: Failure to observe tmux is `unknown`, never evidence of `stopped`. A non-zero or failed `capture-pane` preserves the authoritative stored state and writes a rate-limited `[tmux-observation]` diagnostic containing stderr or the spawn error. A cached session-list miss must be checked with an exact `tmux has-session` probe; permission, socket/server-visibility, and unexpected failures remain `unknown`. Destructive watcher passes require two temporally distinct affirmative missing-session probes before reaping; overlapping calls in one polling window count once. One-shot lifecycle commands may act on one affirmative exact-session miss because their process-local counter cannot survive into another pass. Any successful live or unknown observation resets the pending-miss count. Immediately before teardown, current PID/session/operation metadata is re-read; unavailable or changed metadata aborts cleanup.

**PID observation safety**: Only `ESRCH` is affirmative PID absence. `EPERM` and unexpected signal-0 failures are unavailable observations and fail open as alive. After signal 0 confirms that a PID exists, inability to read its process start time is also `unknown` and fails open as alive. A start time later than `claude_pid_epoch` plus the documented margin remains affirmative evidence of PID reuse, but cleanup must never signal that recycled numeric PID.
...
**Per-agent watchdog**: A failed pane capture does not start the watchdog's tmux-gone grace period. The watchdog first performs an exact `tmux has-session` probe, logs the capture/probe failure, and remains alive for `live` or `unknown`. Only affirmative exact-session absence may start the existing 10-second grace period.
