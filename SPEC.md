...
- **codex role instructions on resume:** the role text is regenerated from the current frozen meta (so `ib sandbox refresh` policy changes are included) and passed again as `-c developer_instructions`. Codex (verified against the 0.154.0 source) does **not** add a second copy: `codex resume` rebuilds history from the rollout, which already holds the spawn-time copy, and restores the saved reference context, so the next turn records only world-state diffs. Plain `developer_instructions` is not part of those diffs, so the regenerated text reaches the model when codex next rebuilds its full initial context — after a compaction — not at the resume itself. The hooks enforce the new policy immediately either way; only the model's description of it can lag.
...
| 15 | **Role text on resume lags until compaction.** A resumed codex session keeps the spawn-time `developer_instructions` in its context; a regenerated value (e.g. after `ib sandbox refresh`) reaches the model only when codex rebuilds its full initial context after a compaction (§18.7). Enforcement is unaffected (hooks read the current meta). A possible follow-up is to deliver changed text through the codex SessionStart hook on resume. |
