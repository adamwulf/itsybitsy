...
IB_SANDBOX_LOG_DIR=$(mktemp -d '<AGENTSDIR>/.ittybitty/agents/agent-claude-snapshot/sandbox-log.XXXXXXXX') || IB_SANDBOX_LOG_DIR=""
...
if [[ "$SETSID" == "setsid" ]]; then
    setsid /bin/sh -c 'if [ -n "$IB_SANDBOX_LOG_DIR" ] && [ -d "$IB_SANDBOX_LOG_DIR" ]; then printf '\''%s\n'\'' "$$" > "$IB_SANDBOX_LOG_DIR/root-request"; n=0; while [ ! -f "$IB_SANDBOX_LOG_DIR/root-ready" ] && [ -d "$IB_SANDBOX_LOG_DIR" ] && [ "$n" -lt 100 ]; do sleep 0.02; n=$((n + 1)); done; if [ ! -f "$IB_SANDBOX_LOG_DIR/root-ready" ]; then printf '\''[%s] [SandboxCollector] ERROR: CLI identity registration unavailable; kernel enforcement remains required\n'\'' "$(date '\''+%Y-%m-%dT%H:%M:%S%z'\'')" >> "$IB_SANDBOX_LOG_AGENT_LOG"; fi; fi; exec "$@"' sandbox-log-gate '/usr/bin/sandbox-exec' -f '<PROFILE>' -D 'SCRATCHPAD=<VALUE>' -D 'GITDIR=<VALUE>' -D 'PROJECTDIR=<VALUE>' -D 'PARENTCLAUDE=<VALUE>' -D 'REPOAGENTS=<VALUE>' -D 'AGENTDIR=<VALUE>' -D 'WORKTREE=<VALUE>' claude --resume "<SESSION-UUID>" --model sonnet --dangerously-skip-permissions 2> "$STDERR_LOG" &
else
