...
**Sandbox contract (2026-09-08, clarified):** kernel sandboxing defaults to true for repository agents and per-repository coordinators. Agent-type Markdown accepts `sandbox: true` / `sandbox: false` or an object with boolean `enabled`, `rawAllow`, and `domains`. The most specific explicit enablement wins across `_all`, applicable `_non_coordinator`, and the parent-first inheritance chain; omission inherits, and only final resolution defaults true. Enabled mode runs the agent under itsybitsy's `sandbox-exec` profile and egress proxy, using the resolved path configuration as the coarse kernel boundary while hooks enforce agent-type tool permissions and fine-grained path rules. Explicit false omits only itsybitsy's kernel wrapper, proxy, and kernel-denial collector, restoring the pre-sandbox launch behavior; the hooks remain authoritative for every tool allow/deny decision and must never hand an approval prompt to the user. Codex/Fugu always use `-a never`, with `-s danger-full-access` under our kernel wrapper and explicit `-s workspace-write` when it is off. Agy always uses `--dangerously-skip-permissions --mode=accept-edits` in both modes, with fail-closed hooks retained. Claude enabled mode adds `--dangerously-skip-permissions`; disabled mode emits neither that flag nor `--permission-mode`. In both modes Claude's PreToolUse hook returns explicit allow/deny before native permission resolution, with `permissions.deny` checked first, so allowed calls do not prompt and denied or unlisted calls are rejected. Spawn freezes policy, resume preserves it, and `ib sandbox refresh` applies changed type settings in either direction. Enabled launches fail closed. The global `@system` coordinator remains unsandboxed. See [the type guide](docs/agent-types/README.md) and [sandbox specification](SPEC-SANDBOX.md).
...
**Worktree caller fast path (2026-09-09):** An existing agent Git worktree in
an OS-account-registered repository is identified from its agent metadata
without inspecting process ancestry. Validate the canonical worktree path,
matching metadata ID, Git administration directory, and worktree backlink
before skipping the no-worktree resolver. Unrelated `worktree: false` agents
must not cause a worktree caller to invoke `/bin/ps`. Other callers retain the
no-worktree ancestry checks, and every resolved agent still passes the same
`metaCanSpawnChildren` gate. Codex's native sandbox stays enabled when the
itsybitsy wrapper is disabled; no escalation or hook execution bypass is used.
...
**`--model` is a user-only override.** Independently of whether the caller may spawn at all, an agent (even a spawn-permitted manager or coordinator) may **not** pin a spawned child's model — the child's model must come from the agent-type layers / config precedence chain (§2 item 3), never a caller-supplied override. Only a human running `ib new-agent` from the CLI (or the TUI new-agent dialog) may set `--model`. Enforcement covers every spawn vector: (1) the agent-path hook (`checkIbCommandAccess`) denies any Bash `ib new-agent … --model …` (and the `ib new` alias) for every agent — the detection is quote/heredoc aware so a prompt body containing the literal text `--model` is not mistaken for the flag; (2) the intercept-task hook (`processTaskIntercept`, `src/hooks/intercept-task.ts`) denies the native `Task`/`Agent` tool's `model` argument when the caller is a resolved agent (the native-spawn equivalent of `--model`); (3) `newAgent()` (`src/ib-commands.ts`) is the defense-in-depth backstop shared by both — when the resolved caller context is a verified agent and `opts.model` is set, the spawn is refused (a human / primary-Claude caller resolves to null and is unrestricted). The backstop runs after the caller spawn-permission gate, so a manager supplying a model is refused specifically for the model override rather than the spawn permission.
...
11. **Write start.sh**: A bash script that:
    ...
    - When kernel sandboxing is enabled, starts a launch-owned denial collector outside Seatbelt, confirms readiness, and registers CLI identity before the wrapper runs. Disabled mode omits these itsybitsy components and restores the pre-sandbox launch path; tool authorization remains hook-driven and prompt-free. Global `@system` remains deferred.
...
14. **Auto-accept workspace trust**: A background process polls tmux output for the startup permission prompts (workspace trust, external CLAUDE.md imports, new MCP server(s)) and sends the **prompt-specific accept sequence** to answer them. Since Claude Code 2.1.259 rebuilt these on a shared confirmation component whose cursor defaults to the *cancel* option ("No, exit" for trust — a bare Enter would QUIT Claude), the sequence is not always a bare Enter: `Down,Enter` for trust and external imports, `Up,Up,Enter` for a single new MCP server, `N Downs,Enter` for the multi-MCP "Enable selected" checklist, and a bare `Enter` for the pre-2.1.259 layouts (see `classifyClaudeStartupPrompt` in `src/coordinator.ts`, shared by the coordinator poll, the per-agent watchdog, and the resume/new-agent helpers). Generic tool/Bash prompts are never auto-accepted. Before each send the CURRENT VISIBLE pane (`-S 0 -E -`, no scrollback) is re-captured and re-classified so a navigation sequence is keyed to the live prompt, never a stale one lingering in history; the coordinator serializes this under the outbox delivery lock and the watchdog under `runSessionExclusive`. Runs up to 5 attempts with 4-second waits between each.
...
2. **No tmux session** → `stopped`, unless `created_epoch` is less than 6 seconds ago → `creating`. The 6-second grace period (`CREATING_GRACE_PERIOD_MS`) is purely time-derived from `created_epoch` in meta.json — not written as a state value. Two different branches reach this step, and the evidence rule differs because only one of them has anything to observe:
   - **meta.json carries no `tmux_session` at all** — the spawn has not reached the tmux step, or never will. There is no session name to probe, so the observation rules below do not apply and this resolves in a SINGLE pass. The spawn log decides first: a `[spawn]` start line within `SPAWN_IN_PROGRESS_WINDOW_MS` with no terminator → `creating` (worktree setup on a large repo runs well past the 6-second grace); otherwise the grace period decides, and a destructive pass may reap on that one observation.
   - **meta.json names a session that is not live** — here **"no tmux session" means an AFFIRMATIVE absence**, not a failed probe, and on a destructive watcher pass it means two temporally distinct ones — see the **Observation safety invariant** below, which governs this branch. A failure to observe never satisfies it.
...
3. **Tmux session exists — check transient overrides** (tmux output parsing, minimal):
   ...
   - If the tmux tail shows a top-level `API Error:` refusal in the last 15 logical lines (after trailing blanks are stripped), it is a **TERMINAL** state the watchdog never retries: a Usage-Policy (AUP) violation (`Claude Code is unable to respond` + a Usage Policy or `…/legal/aup` reference) → `api_terms`; a model input-safety rejection (`<model>'s safeguards flagged this message` + an AUP reference — e.g. Fable's, often a probabilistic false positive whose remediation is to edit the message or switch models, NOT retry) → `api_safeguard` (model-agnostic — the model name is not hardcoded). A recovery-eligible transient `API Error:` instead (idle timeout / 5xx / connection error / "temporarily limiting requests") → `api_error`, which the watchdog DOES auto-retry with backoff (§8.5). Both terminal refusals are checked BEFORE `api_error` so a refusal is never misclassified as recoverable.
   - If stored `meta.state === "waiting"` AND tmux shows a live background task (§8.5.1): a Claude footer marker in the last 8 logical lines (after trailing blanks are stripped), or the Gemini task section below the latest input box (§19.7) → `running`. Scoped strictly to `waiting` — we do NOT override `complete` (intentional sign-off) or `running` (already correct). See §8.5.1.
...
**Observation safety invariant**: Failure to observe tmux is `unknown`, never evidence of `stopped`. A non-zero or failed `capture-pane` preserves the authoritative stored state and writes a bounded `[tmux-observation]` diagnostic containing truncated stderr or the spawn error; changed failure details and a new failure episode after recovery are logged independently. Recovery is scoped to the observation that failed: seeing the session live re-arms only the session-liveness (`has-session`) diagnostics, while the pane diagnostics (`capture-pane`, `list-panes`) re-arm only on a successful pane read. A session that is listed live but whose pane cannot be read is observed live on every poll, so re-arming pane diagnostics on liveness would re-log the identical failure every tick and rotate away the very history the interval protects. A cached session-list miss must be checked with an exact `tmux has-session` probe; permission, socket/server-visibility, and unexpected failures remain `unknown`. Destructive watcher passes require two temporally distinct affirmative missing-session probes before reaping; overlapping calls in one polling window count once. One-shot lifecycle commands may act on one affirmative exact-session miss because their process-local counter cannot survive into another pass. A live or unknown has-session probe RESULT resets the pending-miss count; a pending miss is itself reported as `unknown` WITHOUT resetting the count, or the count could never reach two and reaping would never happen. Immediately before teardown, cleanup takes the same cross-process lifecycle lock used to claim resume/merge operations and write transient state, then re-reads current PID/session/generation/operation metadata while holding exclusion through every signal and `tmux kill-session`. Unavailable or changed metadata, an operation marker, or failure to acquire that lock aborts cleanup. The generation signal is `created_epoch`: a recreated agent can land on the same session name and the same PID/PID-epoch pair, so comparing only those three lets an observation taken against the previous generation tear down the new one. That final read bypasses the meta.json mtime cache — the cache already re-parses on any changed mtime, and bypassing it only at this seam (not in the shared read path) also covers a rewrite landing inside the filesystem's timestamp granularity. A failed tmux teardown logs its bounded stderr/spawn error and exit code; a teardown against a session that is already gone is the expected no-op and is classified as such, not as a failure.

**Lifecycle-lock reclamation**: The cross-process lifecycle lock is reclaimable in exactly two shapes, and both require the generation to be past the stale threshold: one whose body names an owner that is affirmatively gone, and one whose body is definitively not a record at all and so names no owner to check (see the bodyless paragraph below, where mtime is the only age evidence available). Reclaiming one lock generation is serialized by an exclusive, generation-keyed claim, so two contenders can never both remove the same generation — re-reading the owner token immediately before removing it narrows that race but does not close it, and the loser of the narrowed race removes the *successor* lock, leaving two processes believing they hold it. The lock file and the claim file are BOTH published atomically — a fully-written staging file linked into place, never an exclusive create followed by a write — so neither is ever observable half-written. That staging write does not create parent directories: acquiring under a removed agents tree fails rather than resurrecting it, the same as the exclusive create it replaced. An exclusive create publishes a zero-byte file, and a process killed in that window would leave a lock naming no owner: unreclaimable (reclamation cannot check a holder it cannot name) and never deleted by anything, wedging the agent permanently. A claim is never unlinked to take it over; a claim is stepped over by claiming the next slot only when its reclaimer is affirmatively gone AND the claim itself is past the stale threshold, so a reclaimer that dies mid-steal cannot wedge the lock. Both conditions are required so that neither a wrong liveness verdict nor age alone can take another process's claim: a claim is held for microseconds in the normal path, so anything old enough to step over is crash debris. Claims are dated by their recorded `created_at_ms`, falling back to the claim file's mtime for records written before that stamp existed (publishing by `link` leaves the mtime intact), so a legacy claim ages out instead of blocking its generation forever. Exhausting the claim slots, an unreadable claim, an unreadable lock, a live reclaimer, or a claim too young to be debris all fail safe: the caller reports the agent busy.

Lock and claim records carry a `pid_epoch`, and holders are checked with the rendering-side identity rule rather than a bare liveness probe — otherwise one recycled PID would make a long-dead holder look alive forever and wedge that generation permanently. That check is **uncached**: stepping over a claim or reclaiming a lock hands a generation to a second process, so it may no more be decided from a cached process-start read than a signal may. It is the rendering-side rule made uncached, NOT the stricter destructive rule — the destructive rule treats absent evidence as "not current", which would make every legacy record with no epoch instantly stealable and turn a wedge into a live steal. That epoch is the writer's REAL process start time, deliberately unlike `claude_pid_epoch` / `watchdog_pid_epoch`, which are PID-WRITE stamps taken moments after their process starts: a lock is acquired at an arbitrary point in its holder's life, so a write-time stamp would sit outside the margin and read as recycled immediately. A record written without an epoch, or a live PID whose start time cannot be read, still reads as alive — neither absence is permission to steal.

The lock stale threshold must stay strictly greater than the process-start cache TTL, and that ordering is asserted at module load rather than left to a comment. Reclaiming takes two independent judgments — the generation is old enough, and its holder is gone — and if a cached "holder is gone" observation could outlive the age gate, both could be wrong at the same instant and a live lock would be stealable with no syscall failure anywhere. The gates re-probe uncached, so this ordering is not what makes reclamation safe; it is enforced so it can never quietly become so again.

A lock whose body reads successfully but is definitively not a record (a zero-byte file left by an older binary, or corruption from outside this protocol) is reclaimable through the same claim protocol once its mtime is past the stale threshold, keyed on a generation token derived from its size and mtime. A lock that cannot be READ at all remains unavailable evidence and is never removed.

**PID observation safety**: Only `ESRCH` is affirmative PID absence. `EPERM` and unexpected signal-0 failures are unavailable observations and fail open as alive for state rendering. After signal 0 confirms that a PID exists, inability to read its process start time is also `unknown` and fails open as alive for state rendering. A process start outside the documented margin on either side of its PID-write epoch is affirmative evidence that the numeric PID identifies another generation. Destructive signaling uses a stricter, uncached check immediately before `SIGTERM`: the PID-write epoch and process start must both be available and affirmatively match. The governing rule is that a cached process-start read may RENDER a state but may never AUTHORIZE a teardown, and state detection both renders and reaps: the gate that concludes `stopped` from `claude_pid` re-probes uncached before its reap runs, so a cache entry left by a recycled PID cannot cost a live agent its session. The `tmux kill-session` that tears down a husk is destructive in the same way and takes the same standard, and so is the watchdog `SIGTERM` that would otherwise precede it: when the `stopped` verdict came from reading `claude_pid`, EVERY destructive act on that pass — the watchdog signal as well as the session kill — is withheld outright if that PID is affirmatively live and current. The per-signal identity check cannot stand in for this, because the watchdog of a wrongly-stopped agent is itself live and current and so passes that check; withholding only the session kill leaves a live agent running with no watchdog, and therefore no message delivery and no state updates. Verdicts derived from tmux evidence are deliberately unaffected — an agent whose pane is authoritatively dead still loses its session even though its PID is alive, which is the case that teardown exists for. Missing legacy epochs, unavailable start-time observations, and changed/recycled generations skip the signal. A skipped signal is reported only when it is informative — the PID is alive but has no recorded epoch, an unreadable start time, or a start that no longer matches — and those lines are rate-limited on the same interval as the tmux observation diagnostics. An affirmatively DEAD PID is skipped SILENTLY: nothing was withheld from a live process, and cleanup runs on every pass over every already-stopped agent, so logging it would rotate away the very lifecycle history the interval protects. The per-agent watchdog records its own PID epoch in transient state so it is subject to the same identity rule on BOTH paths: the destructive path uses the strict check before signalling it, and the fast-path that decides whether to TRUST a cached transient snapshot uses the rendering-side check (PID-only for legacy transients with no recorded epoch, fail-open when the start time is unavailable). A bare liveness check on that fast-path would let a recycled PID authorize a stale snapshot for up to the transient freshness window.

**Dead-pane safety**: Pane contents are not authoritative lifecycle evidence. The text `Pane is dead` can legitimately appear while an agent reviews source or test output. Consumers may use that text only as a reason to query tmux's `#{pane_dead}` metadata; only authoritative dead metadata can trigger `stopped` or teardown. The pane probe uses `list-panes -s` and calls the session dead only when every pane across every window in the exact session reports dead.

**Per-agent watchdog**: A failed pane capture does not start the watchdog's tmux-gone grace period. The watchdog first performs an exact `tmux has-session` probe, logs bounded capture/probe details once per failure episode, and remains alive for `live` or `unknown`. A successful capture is also paired with authoritative all-window pane metadata so a readable remain-on-exit pane cannot refresh transient state forever. Affirmative exact-session absence or an all-pane-dead result starts the existing 10-second grace period; unknown pane metadata preserves the watchdog.
...
resolved toggle determines whether the resumed CLI uses the itsybitsy kernel
wrapper or restores its pre-sandbox process behavior. Tool authorization stays
hook-driven and prompt-free in either case. Refresh is an unsandboxed operator
action; agent hooks deny it. Running agents are paused before policy changes, and failed
transitions restore the prior seal where possible or report explicit recovery.
...
**`nuke` is a user-only command.** No agent — worker, manager, coordinator, or `@system` — may run `ib nuke` (single, recursive, or nuke-all). It is a destructive, irreversible teardown reserved for a human at the CLI or the `ib watch` TUI. Enforcement is two-layered, mirroring the `ib new-agent` caller gate (§1a0): (1) the agent-path PreToolUse hook (`checkIbCommandAccess`, `src/hooks/agent-path.ts`) denies any Bash `ib nuke …` up front for every agent — the blanket deny sits ahead of the `@system` early-return so even the system coordinator is blocked; (2) the `ib nuke` CLI dispatch (`src/index.ts`) is a defense-in-depth backstop that resolves the caller (`resolveCallerAgentContext`, `src/ib-commands.ts`) and refuses a verified agent caller (fail-closed on an unverifiable caller). A human / primary-Claude caller resolves to null and is unrestricted. The TUI nuke actions call `nukeAgent` / `nukeAllAgents` in-process (a human driving `ib watch`) and are unaffected. An agent that needs to tear down another agent uses `ib retire` (which remains manager-gated, §1.4) and asks the user for a nuke when a destructive teardown is truly required.
...
| `sandbox` | boolean or object | No | Kernel sandbox switch and rules. Boolean shorthand sets enablement; object form accepts boolean `enabled` plus `rawAllow` and `domains` lists. Most specific explicit enablement wins across applicable layers and ancestors; omission inherits and final resolution defaults to `true`. Lists union independently. Frozen in metadata at spawn; refresh applies updated type settings. Hooks enforce `paths` even when kernel sandboxing is disabled. |
...
### 3.6 `ib merge <id> --keep` — Merge Without Closing

`--keep` lands an agent's **committed** work on the target branch and leaves the agent running so it can keep working on the same branch; its later commits can be landed again the same way. It is a TS-only flag (no bash equivalent) and does not change the behaviour of `ib merge` without it. `--force` may be combined with it and is a no-op either way (the native merge never prompts).

**What runs** (`mergeAgent(agent, targetDir, { keep: true })` → `mergeKeepIntoTarget`):

1. The same op-marker guard as a closing merge (`merging` in `meta.transient.json`; see `docs/plans/long-running-op-locking.md`): the agent renders `merging` while the commit is created, a concurrent merge/merge-check/restart is refused, and the marker is cleared in `finally` so the rendered state falls back to whatever it was.
2. Pre-merge checks §3.2 items 1, 2, 3, 5, 6 and 7 (agent dir exists, worktree exists, caller not inside the agent's worktree, target branch detected, agent branch exists, caller's CWD clean) — **item 4 (agent worktree clean) is skipped**: nothing runs inside the agent's worktree on this path, so in-progress uncommitted edits are simply not part of what lands. Refusing would make the flag unusable in exactly the mid-task situation it exists for. Item 8 (the temp-worktree rebase check) is also skipped — there is no rebase. (The numbering here is §3.2's; the inline step comments in `mergeAgent` use their own 1, 2, 2b, 3… scheme.)
3. Target-branch detection §3.3 (current branch of `targetDir` → main → master).
4. `git log <target>..agent/<id>` — zero commits → succeed with `Nothing to merge: …` and create no commit (a `--no-ff` merge of an already-contained tip would report "Already up to date").
5. `git -C <targetDir> checkout <target>` (same as §3.4 step 2), then **`git -C <targetDir> merge --no-ff agent/<id> -m "<message>"`** — always a real merge commit, regardless of `is_running_as_agent()`. Subject: `Merge agent <id> work into <target> (agent kept running)`; body: commit count, branch, and that later commits can be merged the same way. Distinct from the closing merge's `Merge agent <id> work` so `git log` shows which merges kept the agent.
6. On a non-zero merge exit: `git -C <targetDir> merge --abort` (best-effort — "no merge to abort" when the merge never started), then `git status --porcelain`; if the checkout is still dirty the error carries an explicit `WARNING`. The error names the target as unchanged, includes git's own output (the `CONFLICT` lines), and suggests asking the agent to merge or rebase onto the target and retry.
7. On success: report `Merged agent <id> into <target> at <sha> (<n> commit(s), agent kept running)` and append one line to `agent.log`. **Nothing else** — no tmux capture/kill, no process kill, no worktree or branch removal, no archive, no question removal, no seal removal, no team prune / leave notice (§16.5 applies only to departures), no meta.json write.

**Why a porcelain merge in `targetDir`, and no rebase.** The target branch is checked out in `targetDir` (the primary checkout, or the manager's worktree when a manager runs it). Writing the merge commit with plumbing (`commit-tree` + `update-ref`) behind that checkout would move HEAD while leaving its index and working tree at the old tree. `git merge` in the checkout advances HEAD, index and working tree together, and `git merge --abort` restores all three; the §3.2 refusal on a dirty target checkout is what makes that restore exact. The agent's branch is never rewritten (the closing merge's §3.4 step 1 rebase would rewrite history under a live session), and a fast-forward would leave no merge commit to identify what landed. Because the merge commit's second parent is the agent's tip, a later `--keep` (or the eventual closing `ib merge`, whose rebase drops already-landed patches by patch-id) picks up only newer commits.

**Not affected**: hooks (`agent-path` already treats `ib merge` as manager-only and skips flags when locating the target id; the Stop hook still counts a `--keep`-merged agent as an unfinished sub-agent, which is correct — it must still be merged or retired), the watchdog (`merging` is a render label only; the same transient marker is already used by merge-check on live agents), and `ib watch` (the `m` key remains the closing merge; `--keep` is CLI-only).
...
Repository-agent message inputs stage attachments only when Send is invoked. Draft edits, drag/drop, and cancellation before submission create no temporary files. Absolute file references and explicit dot-relative references are parsed as literal data; relative references resolve from the destination repository root, separately for each recipient in a fan-out. Terminal backslash escaping, single/double quotes, Unicode, repeated paths, and surrounding prose are preserved. URLs and backtick code spans/fences are left unchanged; an unmatched inline backtick is ordinary text. Missing unquoted slash-command names remain slash commands. Directories and other non-regular files are rejected; no recursive copy is implied.

Live project references are preserved instead of snapshotted: a relative path that stays within the project refers to the receiving agent worktree, and an absolute path inside that worktree stays unchanged. This also permits instructions to create future project files. Containment resolves symlink parents; project links to external files still stage. Relative references leaving the project resolve from the destination repository root. Missing external attachments fail with an actionable error; use backtick code spans when discussing literal paths rather than attaching files.
...
Queue acceptance commits the send: a later delivery failure leaves the message queued and its snapshots retained, and must not invite the dashboard to enqueue a duplicate. Accepted snapshots are not removed on delivery acknowledgment; they remain available for queued delivery and later reads until external temporary-directory cleanup. Dashboard team sends with recipients record room history after at least one recipient accepts; a total staging failure records nothing, so retrying does not duplicate room history. Ordinary team sends keep the record-regardless behavior of §17.4. The feature does not update path permissions or restart agents. Global system-coordinator sends and ordinary CLI/agent-to-agent sends retain their existing behavior.
  ...
  .ittybitty/
    ...
    agents/
      <agent-id>/
        ...
        .claude/
          settings.local.json        # Live worktree:false Claude settings, passed with --settings
...
itsybitsy hooks operate across these execution contexts. Each context has different hook installations, permissions, and behavioral constraints:
...
2. Claude `worktree: false` creation writes hooks to `<agent-dir>/.claude/settings.local.json` and passes that file via `--settings`.
3. The isolated worktree or agent directory is removed on retire/merge/nuke.
4. Repository and user settings are never modified by agent lifecycle operations.

**Detection pattern**: The `AGENT_CWD_PATTERN` regex (`/.ittybitty/agents/([^/]+)/repo(/|$)`) distinguishes ordinary worktree agents from primary Claude. A Claude `worktree: false` session cannot use that CWD signal, so its explicitly passed settings bind hook commands to the agent ID and the handlers resolve role/policy from that agent's `meta.json`.
...
1. **Tool permission gate**: Check the merged `permissions.deny` patterns first; any match denies. Otherwise the tool must match at least one `permissions.allow` pattern. Patterns are either exact tool names (`"Read"`) or bash prefix patterns (`"Bash(git status:*)"` — matches Bash tool where command starts with `git status`). After this tool gate and every applicable structural/path check below complete, Claude's PreToolUse handler emits the final explicit allow or deny before native permission resolution, so neither an allowed nor a denied/unlisted call produces a user prompt.
...
**Operator display and generated instructions** use the same normalization: only literal false opts out of repository kernel sandboxing; omitted enablement defaults on, and global `@system` remains off. `ib info` and the dashboard safely render partial path lists. Missing legacy inputs can require operator refresh before an enabled resume passes seal/profile checks. Codex/Fugu spawn, resume, and refresh regenerate the role instructions (`-c developer_instructions`, §18.6) from the current frozen policy; a resumed Codex session usually sees the regenerated text only after its next compaction (§18.7 has the exact rule). Disabled instructions describe the restored pre-sandbox launch configuration plus the still-mandatory, prompt-free hooks.
...
The stop hook does **not** parse tmux output for state detection. It relies solely on `last_assistant_message` for determining state. Tmux-detected states (`compacting`, `rate_limited`) are handled by consumers at read time (see §1.3). However, when the determined state is `running`, the stop hook checks tmux for background tasks (shared footer detector, §8.5.1) to avoid nudging agents that are actively working via background tasks.
...
**Other CLIs.** Codex/fugu and agy agents get the same generated instructions (wrapper stripped, plus the skills catalogue): codex through `-c developer_instructions` (§18.6), agy through an always-on rule file (§19.3). The generated instructions never include the project's or the user's instruction files; every CLI reads those natively (the "Instruction files" table in §18.6).
...
4. Stored `meta.state === "waiting"` AND tmux shows a live background task (Claude footer marker or Gemini task section; §19.7) → `running` (see §8.5.1). Scoped strictly to `waiting` — not applied to `complete` or `running`.
...
1. **Direct background shell** — the agent's own tmux footer shows Claude's `· N shell(s)` segment or legacy `⏵⏵.*·\s\d+\s` (e.g., `⏵⏵ accept edits on · 1 shell`), or Gemini's live timestamped task section below the input box (§19.7), OR
...
- If the system coordinator session dies unexpectedly, the main area TMUX view shows "Session stopped — press R to restart". The `R` key (same as agent resume) triggers `restartSystemCoordinator()` when the system coordinator is selected — a **resume**-based restart that continues the prior transcript. Typing `/restart` or `/respawn` into the coordinator chat box or the `s`-key send dialog is different: it does a **FRESH** restart (`restartSystemCoordinatorFresh()`, cleared marker → no resume), matching the Telegram `/restart` dispatch (§4).
...
Default-enabled `sandbox-exec` wrapping applies to ordinary repository agents and per-repository coordinators, with explicit type overrides. Global coordinator sandboxing is deferred until after that rollout has been exercised; the unfinished implementation and review findings are preserved in the retired `sandbox-system-launch` agent archive. Safe launch adoption, configuration protection, and cleanup across overlapping launches remain follow-up requirements.
...
**Expanded same-repo authority**: Per-repo coordinators may run `ib retire`, `ib rehire`, and `ib reassign` on ANY non-coordinator agent within their own repo, regardless of the target's `manager` field. This is enforced in `checkIbCommandAccess` (src/hooks/agent-path.ts) by checking the caller's `coordinator: true` flag in its own meta.json before falling back to the standard manager/spawner check. The other manager-relationship operations (`merge`, `resume`, `pause`) still require the standard manager/spawner relationship. `nuke` is **not** available to coordinators (or any agent) at all — it is a user-only command (§1.8). Cross-repo `retire`/`rehire`/`reassign` is not permitted through this coordinator bypass. Coordinators cannot retire, rehire, or reassign other coordinators via the bypass.
...
- **Unqualified `Bash` NOT denied** — Claude Code's permission resolution removes the entire Bash tool when unqualified `Bash` appears in the native deny list, which prevents qualified allow patterns like `Bash(ib:*)` from working. Instead, only specific Bash patterns are in the allow list; PreToolUse applies authored denies first and explicitly denies non-matching commands before native permission resolution, without surfacing an approval prompt.
...
Codex has no `permissions.allow/deny` array, no `--allowedTools` / `--disallowedTools` CLI flags. itsybitsy expresses the same agent-type allow/deny lists as a generated PreToolUse hook handler dispatched from `src/index.ts`. The kernel-enabled Codex launch line is:
...
At spawn and resume, Codex/Fugu always use `-a never`, so native approval UI is never part of the agent workflow. Enabled kernel mode pairs it with `-s danger-full-access` inside the itsybitsy `sandbox-exec` and proxy wrapper. Explicit false omits that wrapper, proxy, and collector and explicitly selects `-s workspace-write`, restoring the pre-sandbox Codex confinement. Generated hook flags and `--dangerously-bypass-hook-trust` remain present in both modes; the hook decides every tool allow/deny and defaults to deny. Enabled builders reject incomplete wrappers.

| Resolved itsybitsy kernel policy | Native CLI protection flags |
|---|---|
| Enabled | Codex/Fugu: `-a never -s danger-full-access`; agy: `--dangerously-skip-permissions --mode=accept-edits`; Claude: `--dangerously-skip-permissions`. All run inside Seatbelt and keep their hooks. |
| Disabled | Codex/Fugu: `-a never -s workspace-write`; agy: `--dangerously-skip-permissions --mode=accept-edits`; Claude emits neither `--dangerously-skip-permissions` nor `--permission-mode`. Hooks explicitly allow/deny before native permission resolution, so no CLI uses native user tool prompts. |
...
- In enabled mode, Codex's own sandbox is off (`-s danger-full-access`), so the itsybitsy Seatbelt profile provides the coarse OS walls plus the network deny/allowlist and the PreToolUse hook supplies fine-grained tool/path isolation. In disabled mode, `-s workspace-write` restores Codex's pre-sandbox process boundary, while the same hook still enforces agent-type permissions and finer path rules. Codex's `workspace-write` remains broader than the shared path table on macOS (for example `/private/tmp`, `$TMPDIR`, and `~/.codex/memories`), so it must not be described as equivalent to the hook policy.
...
**apply_patch handling:** PreToolUse fires for both `Bash` and `apply_patch` on v0.135.0. Permission denies are evaluated before allow or path checks. For `apply_patch`, a deny matching the raw `apply_patch` name or either `Write`/`Edit` alias blocks the call. Otherwise an allow for the raw name or either alias grants the tool category; the handler then parses every `*** Add File:`, `*** Update File:`, and `*** Delete File:` target and checks each as a write through the shared path resolver. Any denied target rejects the entire patch. The mandatory default Write/Edit floor therefore continues to admit normal agent patches, subject to path enforcement, while an authored deny can forbid them without changing Codex sandbox flags.
...
- **codex path:** generates a codex-shaped `start.sh` via `buildCodexStartContent()` in `src/codex-spawn.ts`. The codex branch:
  ...
  3. Builds the role instructions via `buildCodexDeveloperInstructions()` — the session-start template (the `<ittybitty>` wrapper stripped via `stripIttybittyWrapper()`) plus the skills catalogue — the codex analog of what `session-start.ts` injects for Claude. They launch as `-c developer_instructions="…"`, which codex adds to the session as a developer message next to the repo's own `AGENTS.md`. **Nothing is written into the worktree**: an earlier design wrote a generated `<worktree>/AGENTS.md`, which clobbered a repo-tracked `AGENTS.md` (codex reads one `AGENTS.md` per directory). The value is free text, so it is encoded by `tomlBasicString()` (TOML 1.0 basic string: `\"`, `\\`, `\b \t \n \f \r`, `\uXXXX` for other controls and U+2028/U+2029) and then shell-quoted as one argument. `renderCodexDeveloperInstructionsPayload()` refuses blank role text and a payload over 120 KiB (`CODEX_DEVELOPER_INSTRUCTIONS_MAX_BYTES`: under Linux's 128 KiB per-argument limit, and room in macOS's 1 MiB `ARG_MAX` for the prompt); spawn and resume check this before launch and fail cleanly (spawn via `cleanupOnFailure`), and spawn logs the payload size to `agent.log`. The `-c` value replaces any `developer_instructions` in the user's `~/.codex/config.toml` for itsybitsy agents; user-wide text belongs in `~/.codex/AGENTS.md` instead (below).
...
**Instruction files (all CLIs).** itsybitsy copies no instruction file into any agent's instructions. Each CLI reads the project and user-wide files itself:

| CLI | Project instructions (read natively) | User-wide instructions (read natively) | itsybitsy's role text |
|---|---|---|---|
| claude | `AGENTS.md` (Claude Code 2.1.277+, when the project has no `CLAUDE.md` / `.claude/CLAUDE.md` / `CLAUDE.local.md` in the working directory or above it), else `CLAUDE.md` | `~/.claude/CLAUDE.md` | SessionStart hook injection |
| codex / fugu | `AGENTS.md` (`AGENTS.override.md` wins in the same directory); not `CLAUDE.md` by default (only if listed in `project_doc_fallback_filenames`) | `~/.codex/AGENTS.md` | `-c developer_instructions` |
| agy | `AGENTS.md` / `GEMINI.md`, plus workspace `.agents/rules/*.md` (or `.agent/`, `_agents/`, `_agent/`) — only `trigger: always_on` rules load unconditionally, frontmatter-less rules are ignored; not `CLAUDE.md` | `~/.gemini/GEMINI.md` | `.agents/rules/ittybitty-agent.md` (§19.3) |

So a repo keeps its instructions in `AGENTS.md`. When a codex/fugu or agy agent is spawned in a worktree that has `CLAUDE.md` (or `.claude/CLAUDE.md`) but no project file that CLI loads unconditionally (`AGENTS.md`; for codex also `AGENTS.override.md`; for agy also `GEMINI.md` or a repo-owned rule file with `trigger: always_on` frontmatter under `.agents/rules/` or the `.agent/`, `_agents/`, `_agent/` aliases — itsybitsy's own rule file does not count, and neither does a frontmatter-less or `model_decision` rule), `missingAgentsMdWarning()` prints a warning (stderr, or the watch log under `ib watch`) and logs it to `agent.log`; the spawn still succeeds. Codex/fugu and agy resume run the same check and log it with a `[resume]` prefix; the warning is advisory and can never fail a spawn or a resume. The warning says the CLI "does not read CLAUDE.md by default" because it does not inspect the user's codex `project_doc_fallback_filenames`. For one user-wide file, keep `~/.claude/CLAUDE.md` as the real file (Claude Code has no user-level `AGENTS.md`, and its Edit/Write tools refuse to write through a symlink) and symlink the others to it: `~/.codex/AGENTS.md -> ~/.claude/CLAUDE.md` and `~/.gemini/GEMINI.md -> ~/.claude/CLAUDE.md` (make sure no `~/.codex/AGENTS.override.md` exists, since codex reads it instead). Under the kernel sandbox the CLI process must read both the link and its target: the embedded `_all.md` floor grants read on `~/.claude` and `~/.codex` to every agent, and agy gets `~/.gemini` as its runtime state root (§6.1), so no profile change is needed.
...
- **codex role instructions on resume:** the role text is regenerated from the current frozen meta (so `ib sandbox refresh` policy changes are included) and passed again as `-c developer_instructions`. Codex (verified against the 0.154.0 source) does **not** add a second copy: `codex resume` rebuilds history from the rollout, which already holds the spawn-time copy. What the first resumed turn does depends on whether the rollout has a saved reference context (`reference_context_item`, rebuilt from the newest surviving `TurnContext` record):
  - **Saved reference context (the normal case — a completed turn since the last compaction):** the next turn records only world-state diffs. Plain `developer_instructions` is not part of those diffs, so the regenerated text reaches the model when codex next rebuilds its full initial context (after a compaction), not at the resume itself.
  - **No saved reference context** (no `TurnContext` recorded yet, the newest segment ends in a compaction that cleared it, or a legacy compaction without replacement history): the first resumed turn injects the full initial context, including the regenerated `developer_instructions`, right away.

  The hooks enforce the new policy immediately either way; only the model's description of it can lag.
- **Legacy codex agents:** a codex agent spawned before role instructions moved to `developer_instructions` still has its generated, untracked `<worktree>/AGENTS.md`, and itsybitsy never touches that file. Its resumed rollout holds the role text ONLY as that file's AGENTS.md instructions (it has no developer copy). Deleting or rewriting the file would make codex tell the model on the next turn that those instructions no longer apply (or were replaced), while the new `developer_instructions` is not yet injected — so the agent would run with no role text until a compaction. Leaving the file keeps the old role text working across resume. After its next compaction such an agent carries the old role text (the file) **and** the new one (`developer_instructions`): about the same text twice. **Migration means a FRESH agent:** retire the pre-change codex agent when convenient and start a new one with `ib new-agent`. `/respawn` / `/restart` (`respawnSelf`) are pause + resume: they keep the same legacy codex session and do NOT migrate it. The leftover file also interacts with rebases onto a branch that tracks `AGENTS.md` (for example this repo after the rename); verified with git: if the agent branch's `.gitignore` lists `AGENTS.md` (this repo's old branches did), the rebase silently replaces the generated file with the repo's `AGENTS.md`, so codex swaps the role text for the project text; if the file is not ignored (other repos), the rebase aborts with "untracked working tree files would be overwritten by checkout". Either case is another reason to start a fresh agent rather than rebase such an agent.
- **Legacy codex rehire refusal:** retirement archives untracked files with `git ls-files --others --exclude-standard`, so where git ignored the legacy `AGENTS.md` (this repo's old branches) it is NOT archived, and a rehire would resume the old codex session without its only role copy. `rehireAgent()` therefore refuses — during validation, before ANY state change (no agent dir, worktree, branch, tmux session, or meta write) — when the archived model is codex/fugu, the archive has a worktree, the archived `start.sh` (written once at spawn, and archived) is missing or has no `developer_instructions=` argument (a pre-change spawn), AND the manifest's `untrackedFiles` has no `AGENTS.md`. The message tells the user to start a fresh agent with `ib new-agent`. A pre-change archive whose manifest did keep `AGENTS.md` (repos that did not ignore it) rehires as before, and so does any post-change archive.
...
| 12 | **Codex model availability is account/server-defined.** `ib` validates only the `<cli>:<model>` selector syntax and passes the model half through to the Codex CLI. `ib list-models` is a discovery list of common selectors, not a spawn-time allow-list. If a ChatGPT-plan or API-key account cannot access a requested model, Codex reports the server-side error in the TUI after the first prompt. |
| 14 | **Inline-`-c` TOML-in-shell path safety.** `<abs ib>` is interpolated into a TOML string literal inside a shell single-quoted argument. `isCodexSafeBinaryPath()` rejects paths containing `'`, `"`, `\`, or control characters. itsybitsy's default install paths are safe; this guards against user-customized installs. The free-text `developer_instructions` payload cannot use that rule; it goes through `tomlBasicString()` + `shellQuote()` instead, and the tests run the real `start.sh`/`resume.sh` against a stub `codex` to prove the received argument decodes byte-for-byte (a malformed TOML value would not fail: codex silently falls back to the raw text). |
| 15 | **Role text on resume can lag until compaction.** A resumed codex session keeps the spawn-time `developer_instructions` in its context. When the rollout has a saved reference context (the normal case), a regenerated value (e.g. after `ib sandbox refresh`) reaches the model only when codex rebuilds its full initial context after a compaction; when it has none, the first resumed turn injects the regenerated value at once (§18.7). Enforcement is unaffected (hooks read the current meta). A possible follow-up is to deliver changed text through the codex SessionStart hook on resume. |
| 16 | **Legacy codex agents keep a generated `AGENTS.md`.** Agents spawned before the move to `developer_instructions` keep their untracked `<worktree>/AGENTS.md` (deliberately never touched — removing it would drop their only role copy until compaction). After compaction they carry the old and new role text. In a rebase onto a branch that tracks `AGENTS.md`, the leftover is either silently replaced (when ignored) or aborts the rebase (when not ignored). Where git ignored it, retirement does not archive it, so `rehireAgent()` refuses such archives before any state change (§18.7). Mitigation: retire such agents and start fresh ones with `ib new-agent`; `/respawn` / `/restart` (pause + resume) keep the legacy session and are not a migration. |
...
| `src/agent-instructions-shared.ts` | `stripIttybittyWrapper()`, `buildSkillsSection()` (shared with agy), `missingAgentsMdWarning()` (spawn-time CLAUDE.md-without-AGENTS.md warning for codex/fugu/agy). |
...
itsybitsy can launch an agent under Google's Antigravity CLI (`agy`) in addition to `claude` and `codex`. Selection is per-agent via the same **`<cli>:<model>`** model string — e.g. `agy:gemini-3.7-flash-low`, `agy:claude-sonnet-4-6`. The slug is the first column of `agy models`, passed verbatim to `--model`. `agy` agents launch the **interactive `agy` TUI inside tmux**, exactly like `claude`/`codex`. Permissions and the shared deny-by-default `paths:` policy are enforced by a generated, fail-closed PreToolUse hook; role instructions are delivered through an always-on rule file in the worktree. Agy always launches with `--dangerously-skip-permissions --mode=accept-edits`, so hook allow/deny decisions never become native user prompts. With kernel sandboxing enabled, it also runs inside the shared `sandbox-exec` + proxy wrapper at spawn and resume. Disabled mode omits only that itsybitsy wrapper, proxy, and collector, restoring the pre-sandbox agy launch behavior. The whole `~/.gemini` state directory is added as an agy-only runtime root, excluded from the claude/codex/fugu tables. The frozen toggle controls wrapping; agy supports both modes. Auth is the user's job (one browser sign-in; credentials in the macOS keyring, not under `~/.gemini`). Non-goals (v1): no headless `-p` loop, no `agy` custom agents (`--agent`), no coordinators under `agy`, no new dashboard panes.
...
| D3 | The PreToolUse hook resolves `meta.paths` plus agy runtime roots (including the agy-only `~/.gemini` write root) through the shared deny-by-default table and does the fine-grained path isolation the coarse kernel walls cannot express. It is retained in both modes and is authoritative for every tool approval; a native user prompt is never a fallback. Hooks are registered in `<worktree>/.agents/hooks.json` under the named hook `ittybitty` for `PreToolUse` (matcher `*`), `PreInvocation`, and `Stop`; each `command` = `<abs ib> hooks agy-<event> <agentId>`, timeout 30. |
...
| D6 | Instructions go in `<worktree>/.agents/rules/ittybitty-agent.md` with frontmatter `trigger: always_on`; body = session-start template (wrapper stripped) + the skills catalogue. No project or user instruction file is copied in: agy reads the repo's `AGENTS.md` / `GEMINI.md` and the user's `~/.gemini/GEMINI.md` natively (§18.6 "Instruction files"). Never `--agent`, never overwrite `AGENTS.md`. (Superseded 2026-09-23: the body used to inline the project `CLAUDE.md` and `~/.claude/CLAUDE.md`.) |
...
- `<worktree>/.agents/rules/ittybitty-agent.md` — the `trigger: always_on` rule file (`buildAgyRulesFile`): the role text plus the skills catalogue, nothing else. agy loads it next to the repo's own `AGENTS.md`.
...
agy gets user-wide instructions only from its own global `~/.gemini/GEMINI.md` (symlink it to `~/.claude/CLAUDE.md` to share one file; the `~/.gemini` runtime root and the `_all.md` `~/.claude` read floor make both the link and the target readable under the kernel sandbox). A spawn in a repo with `CLAUDE.md` but no `AGENTS.md`, `GEMINI.md`, or repo-owned `trigger: always_on` rule prints the same kind of warning as codex (§18.6). An agy agent spawned before 2026-09-23 had the project `CLAUDE.md` inlined in its rule file; resume regenerates the rule file without it, so in a CLAUDE.md-only worktree (for example an itsybitsy branch forked before the rename) that agent loses its project instructions mid-conversation — resume logs the same warning to `agent.log` (and stderr / the watch log) and still succeeds.
...
resolved toggle. Both paths keep `--dangerously-skip-permissions` and
`--mode=accept-edits` plus the fail-closed hooks; enabled mode adds the kernel
wrapper, while disabled mode restores the pre-sandbox process boundary.
...
Gemini background processes keep an otherwise `waiting` agent derived as `running` while the current input box has a separator-bounded task section containing `● [HH:MM:SS] <command> running`, followed by a recognized agy status footer. This uses the shared background-task detector, so watchdog waiting notifications are suppressed and dashboard reads use the same result through both live captures and the transient cache. Stored `complete` remains authoritative. Matching rows above the latest input box are ignored, and the override clears when the row disappears. The agy parser and CLI auto-detection recognize the same live section; the parser checks it after completion and before WAITING/idle. See `SPEC-ANTIGRAVITY-CLI.md` §4.6 for the captured fixture.

Like codex, agy's PRIMARY state comes from the hooks writing `meta.json`; `parseStateForCli`/`parseState` gain an `agy` branch (`parseAgyState`, `isAgyTmuxOutput` in `src/parse-state.ts`) as the tmux fallback/override: `esc to cancel` bottom-left or a Braille-spinner activity line (`⢿`/`⣯`/`⣻` + Running/Generating/Reading…) → running; the completion sentinel → complete; `WAITING` → waiting; the trust card → creating; `? for shortcuts` (or a bare `>` between `────` separators) → waiting (defers to the meta state). `rate_limited`/`api_error`/`compacting` stay `unknown` for agy (strings not captured yet); the claude-shaped override detectors (`isCompacting`/`isRateLimited`/`isApiError`) do not false-positive on agy panes. The dashboard chrome slicer gets an `agy` detector (`findAgyInputChromeLogical`, `src/tui/wrap.ts`): the input box is anchored on the latest `>` and its surrounding `────` separators, guarded by an agy status footer, with no-trim fallback. A background-task section adds a third separator and remains in the status area alongside the model/context footer. `ib state` recognizes `agy --dangerously-skip-permissions` / `agy --conversation` command lines whose cwd is inside a registered worktree as agent processes (same tracked/orphan rules as `claude --resume`).
...
| 9 | **Advisory shell scanning is incomplete (accepted).** The Phase B scanner denies recognizable literal paths outside the shared `paths:` table, so direct `cat ~/.ssh/id_rsa` is no longer the current example of an allowed read. Dynamic expansion, subprocesses, and unrecognized command shapes can evade a string scanner. When enabled, agy runs inside `sandbox-exec`, so the kernel refuses such a read regardless. Disabled mode restores pre-sandbox process confinement and retains the same prompt-free, fail-closed hooks; the scanner is the advisory diagnostic layer, not complete process confinement. |
