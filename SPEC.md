...
**External clients (Ghostty).** Attaching an agent in Ghostty clears the manual window-size override (`clearTmuxWindowSizeOverride`, `resize-window -A`) so the window follows Ghostty's dimensions while attached. When `checkClientAttached` detects the attached→detached transition, it resizes the window back to `PINNED_TMUX_WIDTH`, restoring the pin so the dashboard's reflow assumptions hold again.
