...
- **Pane widths — single source of truth:** All pane width math lives in `src/tui/widths.ts`. Never compute pane widths inline. Use the saved (disk-backed, async) helpers (`getSavedTmuxWidth`, `getSavedSidebarWidth`, `getSavedMainWidth`, `getTmuxWidthForAgent`) for spawn/resume flows; use the dashboard live (sync) getters (`getMainWidth`/`getLeftPaneWidth`/`getRightPaneWidth`/`getRepoCoordinatorWidth` on `DashboardComponent`) for rendering and runtime resize. Formulas: `mainWidth = terminalWidth - sidebarWidth - 1`; `leftPaneWidth = clamp(splitPaneLeftWidth, 1..mainWidth-2)`; `rightPaneWidth = mainWidth - leftPaneWidth - 1`. Coordinators (system + per-repo) render and tmux-size at `mainWidth`; regular agents fit the middle pane only. Per-repo coordinator width depends on terminal+sidebar only — the inner middle/right split does NOT change it.
