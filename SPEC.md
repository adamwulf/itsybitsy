...
Gemini background processes keep an otherwise `waiting` agent derived as `running` while the current input box has a separator-bounded task section containing `● [HH:MM:SS] <command> running`. This uses the shared background-task detector, so watchdog waiting notifications are suppressed and dashboard reads use the same result through both live captures and the transient cache. Stored `complete` remains authoritative. Matching rows above the latest input box are ignored, and the override clears when the row disappears. The agy parser and CLI auto-detection recognize the same live section; the parser checks it after completion and before WAITING/idle. See `SPEC-ANTIGRAVITY-CLI.md` §4.6 for the captured fixture.
