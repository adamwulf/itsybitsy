...
- **Hook nudges**: the Stop hook (`agent-status.ts`) routes its self-nudge and `notify_manager` messages through `sendMessage` (`raw: true`) instead of typing into tmux directly, so a hook nudge can't collide with a concurrent `ib send`.
...
When a member leaves a team — whether via explicit `ib team remove` or via lifecycle teardown (§16.5) — the remaining members receive a leave notice: `[sent by <agent-id> in @<team>]: left the team` (or system-phrased), with `fromAgent` stamped explicitly to the departed id (§16.5 sender-attribution rule — never cwd-auto-detected). [^needs review] Exact wording TBD in review.

**Notice-storm suppression on bulk teardown.** A bulk operation — `nukeAllAgents` (§3), `ib nuke <manager>` (manager + descendants), or any path that tears down many agents at once — would otherwise produce an O(N²) storm: tearing down N agents that share a team fires N leave-notices, each fanned out to up to N−1 recipients. The mechanism that prevents this is the `suppressLeaveNotice` parameter threaded through `teardownAgent` (§16.5 — *not* on `archiveAgent`, which only prunes and returns the pruned `(team, id)` pairs): the per-agent loop **prunes membership but emits no notice**, and the bulk caller (`nukeAgentList`) **accumulates** the returned pairs across the whole loop, then fans out **one summarizing leave notice per affected team** (e.g. `3 members left @<team>`), to that team's *surviving* members only. The coalesced notice's sender is the bulk operation itself (system send, `@system` or the user form), not any single departed id. If a team has no survivors (all members were torn down), no notice is sent (empty recipient set, §16.5 empty-survivor carve-out). [^needs review] The exact summary wording is open; the requirement that bulk teardown must not emit per-agent leave notices is **not** optional. Single-agent teardown of one leaf agent (`ib kill`/`ib merge`/`ib nuke` of a childless agent) leaves `suppressLeaveNotice` false and emits the normal per-agent leave notice stamped `fromAgent: <departed-id>`.
...
**Eager prune — separate the membership *write* from the notice *fan-out*, at two different layers.** This is the subtle part: `archiveAgent` (`agent-lifecycle.ts`) is a **pure filesystem-archival function** — it moves the agent dir and calls `deleteAgentOutbox()` / `deleteAgentTransient()`. It does **not** have the cross-repo `repos` list the notice fan-out needs (to find each affected team's *surviving* members, which may live in other repos) and it runs **once per agent** (the `nukeAllAgents`/`nukeAgent` loops call it per agent via `nukeAgentList`). Putting notice emission inside it is therefore wrong on two counts: it lacks the data, and it would re-storm. The correct layering:

- **Membership write → in `archiveAgent` (per agent, unconditional, no notice).** `archiveAgent` removes this agent's id from every team it belongs to, under the `.teams.lock` (§16.2), beside the existing `deleteAgentOutbox()` call. It emits **no** notice and needs no new data — pruning by agent id only reads/writes `teams.json`. It **records (returns) the set of `(team, removed-agent-id)` pairs** it pruned so the caller can notify.
- **Notice fan-out → at the teardown *orchestration* layer (`teardownAgent` / `mergeAgent` / `nukeAgentList`), which has the `repos` context.** A `suppressLeaveNotice` (batch-mode) boolean threads **through `teardownAgent`** — the shared single-vs-bulk chokepoint that `ib kill`/`ib nuke` of one leaf go through, and that `nukeAgentList` calls per agent:
  - **Single-agent teardown** (`suppressLeaveNotice: false`): after `archiveAgent` returns its pruned `(team, id)` pairs, the orchestration layer fans out one per-agent leave notice per affected team to that team's surviving members.
  - **Bulk teardown** (`suppressLeaveNotice: true`): each per-agent `teardownAgent`/`archiveAgent` call only prunes and returns its pairs; the bulk caller (`nukeAgentList`, seeded by `nukeAllAgents`, or by `ib nuke <manager>` whose descendant set comes from `getDescendantsRecursive`) **accumulates** all pruned pairs across the loop and, after it completes, fans out **one coalesced notice per affected team** to that team's surviving members.
- **Sender attribution — stamp explicitly, do not auto-detect.** A single-agent leave notice's `fromAgent` MUST be set explicitly to the **departed agent's id** (`fromAgent: <departed-id>`, the same explicit-sender path as `--from` in §4.1). cwd-based `resolveSenderId` auto-detection is unusable here — the departed agent's worktree is already gone, and the code emitting the notice is the teardown process, not the departed agent. For a **coalesced** bulk notice (summarizing several departures) the sender is the bulk operation itself — stamped as a system send (the `@system` sentinel, or the no-`fromAgent` user form, §4.1) rather than any one departed id — and the body names the count, e.g. `3 members left @<team>`.

| Departure path | Membership prune (`archiveAgent`) | Notice (orchestration layer) |
|---|---|---|
| `ib merge` (single) | prune + return pruned pairs | `teardownAgent`/`mergeAgent` emits one per-agent leave notice, `fromAgent: <id>` |
| `ib kill` (single leaf) | prune + return pruned pairs | `teardownAgent` emits one per-agent leave notice, `fromAgent: <id>` |
| `ib nuke` (single leaf) | prune + return pruned pairs | `teardownAgent` emits one per-agent leave notice, `fromAgent: <id>` |
| `ib nuke <manager>` (manager **+ descendants** via `getDescendantsRecursive` — structurally bulk) | prune + return pairs per agent, `suppressLeaveNotice: true` | `nukeAgentList` accumulates, emits **one coalesced** per-team notice after the loop |
| `nukeAllAgents` (bulk) | prune + return pairs per agent, `suppressLeaveNotice: true` | `nukeAgentList` accumulates, emits **one coalesced** per-team notice after the loop |
...
| `src/ib-commands.ts` | Team-send fan-out (per-member `sendMessage`, excluding sender, empty-set → "no recipients in @<team>" success). `deliverMessage` prefix extension (`in @<team>`, drains the new `team` field at §4.1.1's drain-time). New commands: `teamCreate`, `teamAdd`, `teamRemove`, `teamList`, `teamDelete`, `roster` (nonexistent-team errors per §16.3). Join-notice + single-agent leave-notice emission with **explicit `fromAgent` stamping** (persist-then-notify, §16.4.1/.2). |
...
| `src/agent-lifecycle.ts` | **`archiveAgent`** prunes the torn-down agent from all teams (under `.teams.lock`), beside `deleteAgentOutbox` — and **returns the pruned `(team, removed-id)` pairs**; it emits NO notice and needs no `repos` data (it lacks it). **`teardownAgent`** gains the `suppressLeaveNotice` (batch-mode) param and owns notice fan-out (it has `repos`): false (single-agent) → emit one per-agent leave notice per affected team to survivors, `fromAgent: <departed-id>`; true (bulk) → no notice, the bulk caller (`nukeAgentList`) accumulates returned pairs and emits the coalesced per-team notice afterward (§16.5/§16.4.2). `ib pause` does NOT prune. |
