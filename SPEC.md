...
At spawn and resume, Codex/Fugu always use `-a never`, so native approval UI is never part of the agent workflow. Enabled kernel mode pairs it with `-s danger-full-access` inside the itsybitsy `sandbox-exec` and proxy wrapper. Explicit false omits that wrapper, proxy, and collector, retaining `-s danger-full-access` so hooks own permissions without a native sandbox rejecting authorized calls. Generated hook flags and `--dangerously-bypass-hook-trust` remain present in both modes; the hook decides every tool allow/deny and defaults to deny. Enabled builders reject incomplete wrappers.
...
| Disabled | Codex/Fugu: `-a never -s danger-full-access`; agy: `--dangerously-skip-permissions --mode=accept-edits`; Claude emits neither `--dangerously-skip-permissions` nor `--permission-mode`. Hooks explicitly allow/deny before native permission resolution, so no CLI uses native user tool prompts. |
