...
**Teams tree initial selection — no-selection symmetry (resolved).** The Teams tree adopts the **same** discipline as the Agents tree: **the Teams tree ALSO starts in no-selection, fully symmetric with the Agents tree.** This is the chosen default (the earlier "start with the first team selected" recommendation is superseded). The user was explicit that each panel starts empty and owns its own selection: *"if a team is selected and I shift+tab back to the Agents panel, then no agents should be selected … this way the selected team/agent stays selected as the user moves between panels."* Starting Teams pre-selected would be **asymmetric with Agents for no reason the user asked for**, and would add a selection the user never made — exactly the force-a-selection problem §17.1 removes from the Agents tree. So the Teams tree mirrors the Agents no-selection rules above:
