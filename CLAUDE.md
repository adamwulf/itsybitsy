...
- **Layout (Phase 42+):** Three-column layout: fixed 60-col sidebar (agent tree + info panel + coordinator) | resizable tmux pane | cycling right pane. See SPEC.md §11–13 for full specification.
- **Focus system (Phase 43+):** Tab/Shift+Tab cycles focus between agent-tree, coordinator, active-agent. Input fields appear when coordinator/active-agent has focus. See SPEC.md §13.
- **Coordinator (Phase 44+):** Auto-spawned `ib-coordinator` tmux session with `Bash(ib:*)` only permissions. See SPEC.md §12.
