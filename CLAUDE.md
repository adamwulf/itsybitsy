...
- **Coordinator System (Phases 47–49 — not started):** Two-tier system: system coordinator (`ib-coordinator` tmux session, `~/.itsybitsy/`, Bash(ib:*) only) + per-repo coordinators (special agents with `coordinator: true` in meta.json, Read/Glob/Grep + ib:*, no Write/Edit). System coordinator shown in sidebar + full-width view when selected. Per-repo coordinators are first entry under their repo in agent tree. See SPEC.md §12.
