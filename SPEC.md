...
4. **Format message**: If a sender is detected, the message is prefixed. Real agent IDs render as `[sent by agent <sender-id>]: <message>`. Sentinel IDs that begin with `@` (e.g. `@system`, `@<repo-name>`) render as `[sent by <sentinel>]: <message>` — the literal word "agent" is omitted because these are not agent IDs.
...
5. Start Claude Code inside the session in interactive mode, passing the system coordinator prompt as a positional argument via `"$(cat coordinator-prompt.txt)"` — the same pattern per-repo coordinators use in their `start.sh`. The tmux session is launched with `bash` as its command (i.e. `tmux new-session -d -s ib-coordinator -c <home> bash`) so the pane always runs bash regardless of the user's default `$SHELL`. This is required because the launch line below uses POSIX `$(…)` command substitution, which fish (a possible default shell on macOS) does not support — fish uses bare `(…)` instead. Per-repo coordinators get this for free via their `#!/bin/bash` shebang in `start.sh`; the system coordinator forces bash explicitly via the `new-session` command. The single `tmux send-keys` call carries the full launch line, e.g. `claude --model <coordinator.model> "$(cat '/Users/<user>/.itsybitsy/coordinator-prompt.txt')"`, run via `Bun.spawn(["tmux", "send-keys", "-t", "ib-coordinator", "<launch-line>", "Enter"])`. All tmux interactions use `Bun.spawn` with array arguments — never shell strings. The shell expands `$(cat …)` when bash interprets the line, so the prompt body lives only on disk and never appears in the tmux command itself (sidesteps backtick / quote / `$` escaping bugs). **Note**: The `-p` flag is NOT used because it runs Claude in non-interactive print mode (exits after one response). The system coordinator must run as an interactive session to accept ongoing input. **No timing race**: because the prompt is on the same launch line, claude reads it from `argv` at startup — there is no window where pre-start input could be discarded. The prompt appears as the first user message in the conversation transcript (visible when the user attaches the tmux session). On `claude --resume <id>` (§12.1.5) the positional arg is **omitted** — the prompt already exists in the prior session's transcript, so re-supplying it would inject a stale duplicate.
...
The system coordinator's prompt is delivered exactly **once per session**, via the path that fits the launch mode:

1. **Fresh launch** — The prompt is passed to `claude` as a positional arg via `"$(cat ~/.itsybitsy/coordinator-prompt.txt)"`, mirroring the per-repo coordinator launch in `src/ib-commands.ts` (`start.sh` assembly, around the `claude --session-id … "$(cat <prompt-file>)"` line). claude reads the prompt from `argv` and treats it as the first user message in the conversation transcript — so when the user attaches the tmux session, they see the kickoff message in their conversation history (not hidden in system-level context). This is the only mechanism that delivers the prompt body on a fresh launch.
2. **Resume (`claude --resume <id>`)** — The positional arg is **omitted**. The prior session's transcript already contains the prompt, and re-supplying it on the command line would inject a stale duplicate. claude restores the conversation from the on-disk JSONL transcript.
3. **SessionStart hook** — The `ib hooks session-start @system` hook (§12.1.7) fires on every session start (fresh and resumed) but does **not** re-inject `SYSTEM_COORDINATOR_PROMPT`. There is no meta.json or worktree-derived role context to add for `@system`, so the hook returns an empty `additionalContext`. The hook is still installed because the four-hook contract is keyed to the agent ID (`@system`) rather than to a per-hook decision — keeping the entry consistent with `hook-check-path`, `intercept-task`, and `hook-permission-denied`. Future role context (if any) can be added without re-engineering.

The prompt content (lives in `~/.itsybitsy/coordinator-prompt.txt`, written by `writeCoordinatorFiles()`):
