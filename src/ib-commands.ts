...
import { isValidModel, isValidToolList, isValidTmuxSession, isValidSessionId, isValidShellPath, shellQuote, isValidAgentId } from "./validation";
...
import { generateAgentName, sanitizeAgentName } from "./generate-agent-name";
...
export async function newAgent(
  ...
  // 9. Generate agent ID
  let id: string;
  let idAutoGenerated = false;
  let autoGenHex = "";
  let rejectedGeneratedName: string | null = null;
  if (coordinatorMode) {
    ...
    const shouldGenerateName = config.generateAgentName?.value !== false;
    if (shouldGenerateName) {
      process.stderr.write("Generating agent name...\n");
      let namePart = "agent";
      try {
        namePart = sanitizeAgentName(await generateAgentName(prompt));
      } catch { /* fall back to "agent" */ }
      if (!isValidAgentId(namePart)) {
        // Defensive guard: sanitizeAgentName only emits [a-z0-9-], which is a strict
        // subset of isValidAgentId's allowlist, so reaching here indicates an unexpected
        // slug escaped sanitization. Stash it for logging after agentDir is created.
        rejectedGeneratedName = namePart;
        namePart = "agent";
      }
      id = `${namePart}-${hex}`;
    } else {
  ...
  if (rejectedGeneratedName !== null) {
    await logSpawn(
      agentDir,
      spawnerAgentDir,
      id,
      `name-generation rejected: isValidAgentId returned false for sanitized slug ${JSON.stringify(rejectedGeneratedName)} — fell back to "agent"`,
    );
  }
  if (useWorktree) {
  ...
  if (idAutoGenerated) {
    await logAgent(agentDir, `Generated agent ID: ${id}`);
  }
