...
- [ ] Merges with `permissions.all.*` config keys and the `permissions.allow/deny` frontmatter in `~/.itsybitsy/agent-types/coordinator.md`. **Merge semantics**: entries are appended; hardcoded deny list always takes precedence over user-configured allow (SPEC §12.2.4). The former `permissions.coordinator.*` config keys have been removed.
