...
export function buildCodexLaunchArgs(input: BuildCodexLaunchArgsInput): CodexLaunchArgs {
  ...
  // macOS only: grant write access to ~/Library/Caches. macOS toolchains
  // (SwiftPM, xcodebuild's manifest loader, Homebrew helpers, Xcode itself)
  // write diagnostics, package manifests, and intermediate artifacts here
  // BEFORE any `-derivedDataPath` redirection takes effect. Without this,
  // even a simple `xcodebuild build` aborts during package resolution with
  // EPERM on ~/Library/Caches/org.swift.swiftpm. The directory is shared
  // user state (every macOS app writes to it), so the trust boundary is no
  // narrower than giving the agent the `ib` binary at all. On Linux this
