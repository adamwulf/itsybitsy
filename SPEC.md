...
2. **No tmux session** → `stopped`, unless `created_epoch` is less than 6 seconds ago → `creating`. The 6-second grace period (`CREATING_GRACE_PERIOD_MS`) is purely time-derived from `created_epoch` in meta.json — not written as a state value. **"No tmux session" here means an AFFIRMATIVE absence**, not a failed probe, and on a destructive watcher pass it means two of them — see the **Observation safety invariant** below, which governs this step. A failure to observe never satisfies it.
...
**Lifecycle-lock reclamation**: The cross-process lifecycle lock is reclaimable only from an affirmatively dead owner that has held it past the stale threshold. Reclaiming one lock generation is serialized by an exclusive, generation-keyed claim, so two contenders can never both remove the same generation — re-reading the owner token immediately before removing it narrows that race but does not close it, and the loser of the narrowed race removes the *successor* lock, leaving two processes believing they hold it. The lock file and the claim file are BOTH published atomically — a fully-written staging file linked into place, never an exclusive create followed by a write — so neither is ever observable half-written. An exclusive create publishes a zero-byte file, and a process killed in that window would leave a lock naming no owner: unreclaimable (reclamation cannot check a holder it cannot name) and never deleted by anything, wedging the agent permanently. A claim is never unlinked to take it over; a claim whose reclaimer is affirmatively gone is stepped over by claiming the next slot, so a reclaimer that dies mid-steal cannot wedge the lock. Exhausting the claim slots, an unreadable claim, an unreadable lock, or a live reclaimer all fail safe: the caller reports the agent busy.

Lock and claim records carry a `pid_epoch`, and holders are checked with the rendering-side identity rule rather than a bare liveness probe — otherwise one recycled PID would make a long-dead holder look alive forever and wedge that generation permanently. That epoch is the writer's REAL process start time, deliberately unlike `claude_pid_epoch` / `watchdog_pid_epoch`, which are PID-WRITE stamps taken moments after their process starts: a lock is acquired at an arbitrary point in its holder's life, so a write-time stamp would sit outside the margin and read as recycled immediately. A record written without an epoch, or a live PID whose start time cannot be read, still reads as alive — neither absence is permission to steal.
