...
- **Pane widths — single source of truth:** All pane width math lives in `src/tui/widths.ts`. Spawn/resume code: use the async `getSaved*` helpers. Dashboard render: use the sync `getLive*` wrappers (or `DashboardComponent.get{Main,Left,Right}Width()`). Per-repo coordinators render at `mainWidth`, same as the system coordinator. **Never compute pane widths inline.** Formula: `mainWidth = terminalWidth - sidebarWidth - 1`; the rest derive from it.
